...
def process_logfile(logfile):
    ...
    def request_at_container(line):
        # Line format:
        # [0.23871622961479164] DEBUG    Container    Container c6i.2xlarge-0-cc1app1-2 received request app_1_req_0_0 from LoadBalancer
        parts = line.split()
        timestamp = float(parts[0][1:-1])  # Extract timestamp
        req_id = parts[7]
        container = parts[4]
        if req_id not in requests:
            ...
            return
        if requests[req_id].get("status", None) == "rejected":
            print("Request was already rejected before arriving at container:", req_id)
            return
    ...
    def rejected_request(line):
        # line format
        # [0.835825830676682] WARNING  RuntimeModelSimple Container c6i.xlarge-10-cc1app0-0 rejected request app0_req_d4935ba5_30 due to being busy (q=0 mode)
        parts = line.split()
        timestamp = float(parts[0][1:-1])
        req_id = parts[7]
        container = parts[4]
        ...
        requests[req_id]["container"] = container
        requests[req_id]["at_container"] = float("nan")
    ...
    def drop_pending_requests():
        ...
        # Remove all requests with None assignment
    ...
    keyword_processing = {
        ...
        "received request": request_at_container,
...
def extract_parameters_from_filename(filename):
    ...
    log_<distribution>_<termination>_<LB>_q<queue_size>[_v<version>].log

    Example: log_poisson_drain_simple_q0_v3.1.log
    """
    ...
    # Check for version suffix (e.g., v3.1)
    version = None
    if len(parts) == 5 and parts[4].startswith("v"):
        version = parts[4]  # Extract version (e.g., "v3.1")
        parts = parts[:4]  # Remove version from parts for standard processing
    elif len(parts) != 4:
        # Unexpected format
        return None
